...
func (s *Server) Start() {
	...
	// Init suffix
	var suffixDN *schema.DN
	if suffixDN, err = schema.ParseDN(s.schemaRegistry, s.config.Suffix); err != nil {
		log.Fatalf("alert: Invalid suffix: %s, err: %+v", s.config.Suffix, err)
	}
	s.Suffix = suffixDN

	// Init DB
	repo, err := repo.NewRepository(s.config.DBRepositoryConfig, s.schemaRegistry)
	if err != nil {
...
func (s *Server) RefreshCache(ctx context.Context) error {
	return s.repo.(*repo.DefaultRepository).RefreshCache(ctx)
}

func (s *Server) LoadSchema() {
	s.schemaRegistry = schema.NewSchemaRegistry(s.config.SchemaConfig)
}
...
func (s *Server) NormalizeDN(dn string) (*schema.DN, error) {
	return schema.NormalizeDN(s.schemaRegistry, dn)
}
