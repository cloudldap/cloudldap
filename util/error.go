...
func (e *LDAPError) IsAttributeOrValueExists() bool {
	return e.Code == ldap.LDAPResultAttributeOrValueExists
}
...
func NewNamingViolation(attrName string) *LDAPError {
	return &LDAPError{
		Code: ldap.LDAPResultNamingViolation,
		Msg:  fmt.Sprintf("naming attribute '%s' is not present in entry", attrName),
	}
}
...
func NewNamingViolationForValue(attrName string) *LDAPError {
	return &LDAPError{
		Code: ldap.LDAPResultNamingViolation,
		Msg:  fmt.Sprintf("value of naming attribute '%s' is not present in entry", attrName),
	}
}
