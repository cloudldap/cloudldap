...
import (
	...
	"github.com/cloudldap/cloudldap/server"
...
func runTestCases(t *testing.T, tcs []Command) {
	truncateTables()
	if err := testServer.RefreshCache(context.Background()); err != nil {
		t.Errorf("Unexpected refreshing cache error before running test, got error: %+v", err)
	}
...
func (a AssertResponse) AssertResponse(conn *ldap.Conn, err error) error {
	if a.expect == 0 {
		if err != nil {
			return errors.Errorf("Unexpected error response code. want: no error got: %v", err)
		}
	} else {
		if !ldap.IsErrorWithCode(err, a.expect) {
			return errors.Errorf("Unexpected error response code. want: %d got: %v", a.expect, err)
		}
	}
	return nil
}
...
func setupLDAPServer() *server.Server {
	// customSchema = []string{
	...
	testServer = server.NewServer(&server.ServerConfig{
		DBRepositoryConfig: &repo.DBRepositoryConfig{
			...
			DBMaxOpenConns: 2,
			DBMaxIdleConns: 1,
			ServerID:       fmt.Sprintf("%s:%d", hostname, testPGPort),
		},
...
INSERT INTO entry VALUES (0, gen_random_uuid(), 1, 0, ARRAY []::BIGINT[], TRUE, 'dc=0', 'dc=0', '{"dc":["0"], "objectClass": ["dcObject"]}') ON CONFLICT DO NOTHING;
	`)
	if err != nil {
		log.Fatal("inserting root of root error:", err)
	}
}
