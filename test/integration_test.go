...
func TestModRDN(t *testing.T) {
	...
	tcs := []Command{
		...
		// Rename with old RDN
		ModifyDN{
			"uid=user1-rename", "ou=Users", // remains
			"uid=user1-rename2",
			false,
			"",
			false,
		...
		ModifyDN{
			"uid=user1", "ou=Groups",
			"uid=user1-rename", // already exists as remaining value
			false,
			"ou=Users",
			false,
			&AssertRename{},
...
func TestAssociationWithCustomSchema(t *testing.T) {
	...
	testServer.Config().CustomSchema = customSchema
	testServer.LoadSchema()
	defer func() {
		testServer.Config().CustomSchema = []string{}
		testServer.LoadSchema()
	}()
